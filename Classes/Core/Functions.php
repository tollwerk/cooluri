...
class Functions
{
    ...
    */
        ...
        foreach ($ch as $type => $c) {
            ...
            switch ($type) {
                case 'match':
                    ...
                    break;
                case 'compare':
                    if (isset($ch->value) && !self::compare($value, $c, $ch->value)) {
                        return false;
                    }
                    break; // we don't use empty, because value can be 0
                case 'type':
                    if (!self::validateType($value, $c)) {
                        return false;
                    }
                    break;
            }
    ...
    public static function prepareParamsForCache($params, $tp = 'link_')
    {
        ...
        return DB::escape(serialize($newparams), $tp.'cache');
    }
    ...
    {
        if (empty($conf->cache) || empty($conf->cache->tablesprefix)) {
            return 'link_';
        }
    ...
    public static function pageNotFound($lConf)
    {
        if (!empty($lConf->cache->pagenotfound)) {
            switch ((string)$lConf->cache->pagenotfound->behavior['type']) {
                case 'message':
                    $res = (string)$lConf->cache->pagenotfound->behavior;
                    break;
                case 'page':
                    $res = file_get_contents((string)$lConf->cache->pagenotfound->behavior);
                    break;
                case 'redirect':
                    header((string)$lConf->cache->pagenotfound->status);
                    self::redirect((string)$lConf->cache->pagenotfound->behavior);
                    break;
                case 'userfunc':
                    self::simple_user_func((string)$lConf->cache->pagenotfound->behavior);
                    exit;
                    break;
                default:
                    $res = '';
    ...
    public static function getCoolParams($lConf)
    {
        ...
        if (!empty($lConf->pagepath->id)) {
            $ret[(string)$lConf->pagepath->id] = true;
        }
        if (!empty($lConf->pagepath->saveto)) {
            $ret[(string)$lConf->pagepath->saveto] = true;
        }
