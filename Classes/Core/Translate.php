...
class Translate
{
    ...
    private function removeUriParts($uri)
    {
        if (!empty(self::$conf->removeparts) && !empty(self::$conf->removeparts->part)) {
            ...
            if (!empty(self::$conf->removeparts['redirect']) && self::$conf->removeparts['redirect'] == 1) {
                if (!empty($uri) && $uri != $originaluri) {
                    Functions::redirect(Functions::prepareforRedirect($uri, self::$conf));
                }
            }
        }
    ...
    private function lookUpInCache($uri)
    {
        ...
        if (!empty(self::$conf->cache) && !empty(self::$conf->cache->usecache) && (string)self::$conf->cache->usecache == 1) {
            ...
            $xuri       = $tempuri[0];
            $tempuri[0] = preg_match('~/$~', $tempuri[0]) ? substr($tempuri[0], 0,
                strlen($tempuri[0]) - 1) : $tempuri[0].'/'; // add or remove trailing slash
...
//            $row = $db->fetch($q);
            $row = $db->queryAndFetchStatement('SELECT * FROM '.$tp.'cache WHERE url='.$db->escape($xuri).' OR url='.$db->escape($tempurix));
            if ($row) {
                if (strcmp($row['url'], $xuri) !== 0) { // we've got our $tempuri, not $url -> let's redirect
                    Functions::redirect(Functions::prepareforRedirect($row['url'].(empty($tempuri[1]) ? '' : '?'.$tempuri[1]),
                        self::$conf), 301);
                } else {
                ...
                if (!empty($coolpart)) {
                            ...
                            foreach ($coolparts as $ck => $cp) {
                                ...
                                if ($par) {
                                    ...
                                    if (empty($part->lookindb) || !$cantranslate) {
                                        ...
                                        $res = $db->query(preg_replace('~^'.$db->escape($part['key']).'$~',
                                            (string)$part->lookindb->from, $cp));
                                        $row = $db->fetch_row($res);
                                        // we return value only if we found something
                                        if (!empty($row[0])) {
                                            $finaluriparts[(string)$part->parameter] = $row[0];
                                        }
                                    }
                            ...
                            if (!empty($vm->value)) {
                                foreach ($vm->value as $val) {
                                    if (in_array((string)$val['key'], $coolparts)) {
                                        ...
                                        $key                                   = array_search((string)$val['key'],
                                            $coolparts);
                                        unset($coolparts[$key]);
                                    }
                                }
                    ...
                    if (!empty($coolparts) && !empty(self::$conf->uriparts) && !empty(self::$conf->uriparts->part)) {
                        ...
                        for ($i = 0, $j = 0; $i < count($coolparts) && $j < count(self::$conf->uriparts->part); $i++, $j++) {
                            if (empty($coolparts[$i])) {
                                continue;
                            } // we don't have a item on this key, shouldn't happen
                            ...
                            if (!empty(self::$conf->uriparts->part[$j]->parameter)) {
                                ...
                                if (!empty(self::$conf->uriparts->part[$j]->lookindb->translatefromif)) {
                                    $cantranslate = Functions::constraint($coolparts[$i],
                                        self::$conf->uriparts->part[$j]->lookindb->translatefromif);
                                }
                                ...
                                if (!empty(self::$conf->uriparts->part[$j]->lookindb) && $cantranslate) {
                                    ...
                                    if (preg_match_all('~\$[^ ]+ ~', $sql.' ', $vars)) {
                                        if (!empty($vars[0])) {
                                            foreach ($vars[0] as $var) {
                                                if (isset($nottranslated[substr(trim($var), 1)])) {
                                                    $proceed = false;
                                                } // this var wasn't found in db b4, no need to try to translate
                                                else {
                                                    if (empty($finaluriparts[substr(trim($var), 1)])) {
                                                        continue;
                                                    } // subst not found - query res would be empty
                                                    else {
                                                        $sql = str_replace(trim($var),
                                                            $db->escape($finaluriparts[substr(trim($var), 1)]), $sql);
                                                    }
                                                }
                                            }
                                        }
                                    ...
                                    if ($lastonthepath == null || !empty($finaluriparts[$lastonthepath])) {
                                        ...
                                        if ($lastonthepath == null) {
                                            $sql .= ' AND '.(string)self::$conf->pagepath->start->param.'='.(string)self::$conf->pagepath->start->value;
                                        } else {
                                            $sql .= ' AND '.(string)self::$conf->pagepath->connection.'='.$db->escape($finaluriparts[$lastonthepath]);
                                        }
                                        $res = $db->query($sql);
                                        if (!$db->error() && $db->num_rows($res) > 0) { // no match found - not translating
                                            $row                                                               = $db->fetch_row($res);
                ...
                if (!empty(self::$conf->pagepath->saveto)) {
                    ...
                    if (!empty(self::$conf->pagepath->required) && !empty(self::$conf->pagepath->required->param)) {
                        foreach (self::$conf->pagepath->required->param as $p) {
                            ...
                            if (empty($finaluriparts[$p])) {
                                $rqok = false;
                            } elseif (!empty($paramconstraints[$p]) && !Functions::constraint($finaluriparts[$p],
                                    $paramconstraints[$p])) {
                                $rqok = false;
                            } elseif (!empty(self::$conf->pagepath->allparamconstraints)
                                      && !Functions::constraint($finaluriparts[$p],
                                    self::$conf->pagepath->allparamconstraints)) {
                                $rqok = false;
                            }
                        }
                    }
                    ...
                    if ($rqok) { // only if required is OK
                        ...
                        foreach (array_reverse($px) as $p) {
                            if (empty($p['pagepath']) || $p['pagepath'] != '1') {
                                continue;
                            }
                            $p = (string)$p->parameter;
                            if (!empty($finaluriparts[$p])) {
                                if (!empty($paramconstraints[$p]) && !Functions::constraint($finaluriparts[$p],
                                        $paramconstraints[$p])) {
                                    continue;
                                }
    ...
    public function params2cool(
        array $params,
        $file = '',
        $entityampersand = true,
        $dontconvert = false,
        $forceUpdate = false
    ) {
        if (!empty(self::$conf->cooluris) && self::$conf->cooluris == 1 && !$dontconvert) {
            ...
            $path = $this->saveInCache($uri->params, $path, $uri->originalparams, $updatecacheid, $cacheduri,
                $entityampersand);

            return Functions::prepareforOutput($path, self::$conf).$this->transformParamsToQS($uri->params,
                    $entityampersand);
        } else {
            return (empty($file) ? $_SERVER['PHP_SELF'] : $file).(empty($params) ? '' : '?'.http_build_query($params,
                        '', $entityampersand ? '&amp;' : '&'));
        }
    }
...
//            $row    = $db->fetch($q);
            $row = $db->queryAndFetchStatement(
                'SELECT *, DATEDIFF(NOW(),tstamp) AS daydiff FROM '.$tp.'cache WHERE params='.$cacheQ,
                $tp
            );
            ...
            foreach (self::$conf->predefinedparts->part as $ppart) {
                if (isset($uri->params[(string)$ppart->parameter])) {
                    ...
                    if ($uf !== false) {
                        ...
                        $uri->predefparts[(string)$ppart->parameter] = preg_replace('~\([^)]+\)~',
                            empty($ppart->lookindb) ? $value :
                                Functions::lookindb($ppart->lookindb->to, $value, $ppart->lookindb,
                                    $uri->originalparams), $ppart['key']);
                    } elseif ($ppart->value == $value) {
                        $uri->predefparts[(string)$ppart->parameter] = empty($ppart->lookindb) ? (string)$ppart['key'] : Functions::lookindb($ppart->lookindb->to,
                            $value, $ppart->lookindb, $uri->originalparams);
                    }
    ...
    private function translatePagepath(URI $uri)
    {
        if (!empty(self::$conf->pagepath) && !empty(self::$conf->pagepath->saveto) && !empty($uri->params[(string)self::$conf->pagepath->saveto])) {
            ...
            if ($uf === false) {
                ...
                $db      = DB::getInstance();
                $limit   = 10;
                while ($limit > 0 && (empty(self::$conf->pagepath->idconstraint) ? $result : ($result && Functions::constraint($curid,
                            self::$conf->pagepath->idconstraint)))) {
                    --$limit;
                    if (empty(self::$conf->pagepath->alias)) {
                        $sel = (string)self::$conf->pagepath->title;
                    } else {
                        $sel = (string)self::$conf->pagepath->alias;
                    }
            ...
            foreach (self::$conf->uriparts->part as $pp) {
                if (isset($uri->params[(string)$pp->parameter])) {
                    ...
                    if ($uf !== false) {
                        ...
                        $uri->translatedpagepath[(string)$pp->parameter] = (empty($pp->lookindb) ? $uri->params[(string)$pp->parameter] : Functions::lookindb($pp->lookindb->to,
                            $uri->params[(string)$pp->parameter], $pp->lookindb, $uri->originalparams));
                    }
